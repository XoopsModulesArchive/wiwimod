...
/*
 ...
 require_once __DIR__ . '/source2doc.php';
...
function source2doc($filename)
{
    ...
    for ($i = 0, $iMax = count($const); $i < $iMax; $i++) {
        $html .= '<tr>' . endl;
    ...
    for ($i = 0, $iMax = count($imports); $i < $iMax; $i++) {
        $html .= '<tr>' . endl;
    ...
    for ($i = 0, $iMax = count($vname); $i < $iMax; $i++) {
        $html .= '<tr>' . endl;
    ...
    for ($i = 0, $iMax = count($funcname); $i < $iMax; $i++) {
        $html .= '<tr>' . endl;
        ...
        foreach ($temp as $val) {
            if (mb_strstr($val, '@desc')) {
                $regexp = '|.*?@desc(.*)|si';

                preg_match($regexp, $val, $temp2);

                $desc = $temp2[1];
            } elseif (mb_strstr($val, '@return')) {
                $regexp = '|.*?@return(.*)|si';
