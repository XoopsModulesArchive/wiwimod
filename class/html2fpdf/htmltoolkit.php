...
function ConvertColor($color = '#000000')
{
    ...
    if ('#' == $color[0]) { //case of #nnnnnn or #nnn
        $cor = mb_strtoupper($color);

        if (4 == mb_strlen($cor)) { // Turn #RGB into #RRGGBB
...
function ConvertSize($size = 5, $maxsize = 0)
{
    ...
    if (mb_strstr($size, 'px')) {
        $size *= 0.2645;
    } //pixels

    elseif (mb_strstr($size, 'cm')) {
        $size *= 10;
    } //centimeters

    elseif (mb_strstr($size, 'mm')) {
        $size += 0;
    } //millimeters

    elseif (mb_strstr($size, 'in')) {
        $size *= 25.4;
    } //inches

    elseif (mb_strstr($size, 'pc')) {
        $size *= 38.1 / 9;
    } //PostScript picas

    elseif (mb_strstr($size, 'pt')) {
        $size *= 25.4 / 72;
    } //72dpi

    elseif (mb_strstr($size, '%')) {
...
function AdjustHTML(&$html, $usepre = true)
{
    ...
    if ($usepre) { //used to keep \n on content inside <pre> or <textarea>
        ...
        while (0 != $thereispre) { //Recover <pre>content</pre>
        ...
        while (0 != $thereistextarea) { //Recover <textarea>content</textarea>
    ...
    } // changes &nbsp; and the like by the respective char
