<?php
/*
 * Module and admin language definition
 *
 *	_BTN	: text within buttons or action links
 *  _COL	: column headers
 *  _TXT	: "verbose" text (probably within content)
 *  _FLD	: title of form elements
 *  _DESC	: description under the title for form elements
 *  _MSG	: messages, alerts ...
 *
 */

define('_MB_WIWI_WIWIHOME', 'WiwiHome');
define('_MB_WIWI_PAGENOTFOUND_MSG', "<br>Cette page n'existe pas dans la base documentaire.<br>");
define('_MB_WIWI_NOREADACCESS_MSG', '<br><h4>Désolé, page en accès restreint.</h4><br>');
