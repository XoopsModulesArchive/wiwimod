...
if ('' == $page) {
    redirect_header('javascript:history.go(-1)', 2, _AM_WIWI_NOPAGESPECIFIED_MSG);
}

$pageObj = new WiwiRevision((0 == $id ? $page : ''), $id);
...
foreach ($hist as $i => $rev) {
    ...
    echo '<td><a href="#" onclick="javascript:submitaction(\'page=' . $encodedKeyword . '&op=history&id=' . $rev['id'] . '\');">' . htmlspecialchars($rev['title'], ENT_QUOTES | ENT_HTML5) . '</a></td>';

    echo '<td>' . $rev['lastmodified'] . '</td>';

    echo '<td>' . getUserName($rev['u_id']) . '</td>';

    echo '<td><a href="#" onclick="javascript:submitaction(\'page='
         . $encodedKeyword
         . '&op=history&id='
         . $rev['id']
         . '\');">'
         . _MD_WIWI_VIEW_BTN
         . '</a> | <a href="#" onclick="javascript:submitaction(\'page='
         . $encodedKeyword
         . '&op=diff&id='
         . $rev['id']
         . '\');">'
         . _MD_WIWI_COMPARE_BTN
         . '</a> | <a href="javascript:submitaction(\'op=restore&id='
         . $rev['id']
         . '\');">'
         . _MD_WIWI_RESTORE_BTN
         . '</a> | <a href="javascript:submitaction(\'page='
         . urlencode($encodedKeyword)
         . '&amp;op=fix&amp;id='
         . $rev['id']
         . '\');">'
         . _MD_WIWI_FIX_BTN
