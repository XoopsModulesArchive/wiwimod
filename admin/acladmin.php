...
switch ($op) {
    ...
    $prid = $_GET['profile'] ?? null;
        ...
        $aclComments->addOptionArray([0 => _AM_WIWI_COMMENTS_NONE_OPT, _WI_READ => _AM_WIWI_READERS_OPT, _WI_WRITE => _AM_WIWI_WRITERS_OPT, _WI_ADMIN => _AM_WIWI_ADMINISTRATORS_OPT]);
        $aclHistory = new XoopsFormSelect(_AM_WIWI_HISTORY_FLD, 'prf_historylevel', $prf->historylevel);
        $aclHistory->addOptionArray([0 => _AM_WIWI_HISTORY_NONE_OPT, _WI_READ => _AM_WIWI_READERS_OPT, _WI_WRITE => _AM_WIWI_WRITERS_OPT, _WI_ADMIN => _AM_WIWI_ADMINISTRATORS_OPT]);
    ...
    case 'save':
        if (isset($_POST)) {
            ...
            if (isset($prf_readers)) {
                $prf->readers = $prf_readers;
            } else {
                $prf->readers = [];
            }

            if (isset($prf_writers)) {
                $prf->writers = $prf_writers;
            } else {
                $prf->writers = [];
            }

            if (isset($prf_administrators)) {
                $prf->administrators = $prf_administrators;
            } else {
                $prf->administrators = [];
            }
    ...
    case 'confirmdelete':
        if (isset($_GET['profile'])) {
            ...
            redirect_header('acladmin.php', 2, _AM_WIWI_ERRDELETE_MSG);
        }
