...
function diff_rek(&$a1, &$a2, $D, $k, &$vbck)
{
    ...
    if (0 == $D) {
        ...
        return [array_slice($a1, 0, $x), array_fill(0, $x, '=')];
    }

    $x2 = $vbck[$D - 1][$k + 1] ?? 0;

    $y2 = ($vbck[$D - 1][$k - 1] ?? 0) - ($k - 1);
...
function diffDisplay($source1, $source2)
{
    ...
    foreach ($result[0] as $nr => $line) {
        ...
        if ('' != trim($line)) {
            ...
            $table .= '<tr><td align="center" style="background-color:' . $color . '">' . $sign . '</td><td>&nbsp;</td><td>' . (('' != $color) ? '<strong style="color: ' . $color . ';">' : '') . $line . (('' != $color) ? '</strong>' : '') . '</td></tr>';
        }
    }
